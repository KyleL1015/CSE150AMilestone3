...
Conclusion:
Our Gaussian Hidden Markov Model (HMM) predicts weather states based on observed data where it acheived an accuracy of 82.94% on the test set. This shows that the model is able to capture underlying weather patterns and generalizing well to unseen data. We initially attempted to catergoize the continuous data into discrete bins to utilize the multinomial HMM from hmlearn, however these modifications of the data led to the model not performing well. The confusion matrix reveals some key misclassifications. The model misclassified all drizzle and fog days as sun, and all 8 snow days as rain. On the other hand, it performed well in identifying sun and rain which makes sense as they were the majority of the data. For sun, it correctly predicted all of them as sun and for rain, it correctly predicted 112 out of 120 rain days, with only eight misclassified as sun. These errors suggest that snow and fog were underrepresented in the dataset, making it difficult for the model to learn distinguishing patterns. Also, the provided features may not have been sufficient to separate certain weather conditions effectively. Due to the accuracy our model achieved with the given dataset, the agent's performance did well, but of course there can be room for improvement. Future work to improve our model could be further training on a much more extensive dataset and perhaps modifying some of the parameters of the Gaussian HMM that the hmlearn library provides.
